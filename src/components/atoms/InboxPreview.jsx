...
function InboxPreview({ user, chatTitle, date, lastMessage, className, ...rest }) {
    let users = user
    return (
        <div
            {...rest}
            className={clsx('flex flex-row w-full py-6 items-start border-b border-[#828282] cursor-pointer', className)}>
            {
                users.length > 1
                    ?
                    <>
                    ...
                    <>
                        ...
                        <div className="w-[34px] h-[34px] mr-[23px] rounded-full px-[11px] py-[5px] bg-[#2F80ED] text-white">{users[0][0]}</div>
                    </>
            }
            <div className='text-left max-w-[600px]'>
                ...
                {/* Message preview */}
                <p className='text-[14px] truncate'>{lastMessage}</p>
            </div>
        </div>
    )
}
